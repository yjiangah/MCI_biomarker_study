######
###### Code for association tests between protein levels and AD/MCI/related endophenotypes
#-----------------------------------------------------------------------------------------------------------------------
library(som)
library(robustbase)
...
Merge_Protein_data=Merge_data[c(24:1219)]
...
rownames(Phenotype_test_summary)=names(Data_for_test[c(24:1219)])

for (i in 1:length(Data_for_test[1,c(24:1219)]))
{
  tryCatch({
...
#-----------------------------------------------------------------------------------------------------------------------
### Test of candidate protein levels ~ MoCA / Hippocampal / Gray matter / pTau217 / pTau181 / NfL / Ab42.40
setwd('/PATH')
Merge_data = read.csv('./Merged_endophenotype_data_for_asso_test.csv', header = T)
## Normalization of protein levels
Data_pheno=Merge_data[c(1,12:20)]
Data_protein=Merge_data[c(2:3,5:7,10:11,21,24:41)]
...
Phenotype_test_summary=data.frame(matrix(ncol=28,nrow=length(Data_for_test[1,c(19:36)])))
colnames(Phenotype_test_summary)<-c("Gray_matter_Estimate","Gray_matter_SE","Gray_matter_T_value","Gray_matter_P_value","Hippocampu_Estimate","Hippocampu_SE","Hippocampu_T_value","Hippocampu_P_value","Ptau217_Estimate","Ptau217_SE","Ptau217_T_value","Ptau217_P_value","Ptau181_Estimate","Ptau181_SE","Ptau181_T_value","Ptau181_P_value","Nfl_Estimate","Nfl_SE","Nfl_T_value","Nfl_P_value","Ab42.40_Estimate","Ab42.40_SE","Ab42.40_T_value","Ab42.40_P_value","MoCA_Estimate","MoCA_SE","MoCA_T_value","MoCA_P_value")
rownames(Phenotype_test_summary)=names(Data_for_test[c(19:36)])

for (k in 1:7){
  Data_to_test=Data_for_test[c(k+10,2:10,19:36)]
...
Phenotype_test_summary$Ptau217_FDR=p.adjust(Phenotype_test_summary$Ptau217_P_value,method='fdr')
